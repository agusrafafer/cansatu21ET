 ...
 * gráfico para los datos de la presión.
 *
 * @author agustin
 */
public class PresionGrafico extends Grafico {
    ...
    public ChartPanel graficar(List<FormatoCsv> listaValores, Component panel, String nombreSerie) {
        ...
        for(Object rowKey : dataset.getRowKeys()){
            if(chartPanel != null && !rowKey.toString().contains("[P]")){
                return chartPanel;
            }
        }
        ...
        for (int i = 0; i < listaValores.size(); i++) {
            ...
            dataset.addValue(valor.getPresion(), nombreSerie.toUpperCase() + "[P]", Integer.toString(i + 1));
        }
        if (chartPanel == null) {
